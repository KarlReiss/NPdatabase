  ...
  >
    <div class="max-w-[1200px] mx-auto px-6 py-6">
      <nav class="text-xs text-slate-500 mb-4 flex items-center space-x-2">
        ...
        <span>/</span>
        <span class="text-slate-700 font-medium">疾病列表</span>
      </nav>
            ...
            <thead>
              <tr style="background: var(--theme-bg);">
                ...
                <th class="p-3 text-sm font-bold text-slate-700 border-b">
                  <button class="flex items-center space-x-1" @click="toggleSort('diseaseName')">
                    <span>疾病名称</span>
                    <SortIcon :active="sortKey === 'diseaseName'" :dir="sortDir" />
                  </button>
                </th>
...
const matchesQuery = (item: Disease, query: string) => {
...
const filteredDiseases = computed(() =>
  sortedDiseases.value.filter((disease) => matchesQuery(disease, listQuery.value))
);
